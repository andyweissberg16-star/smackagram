...
def render(level: int = 4, context: str = "recap") -> str:
    ...
    if is_recap:
        out.append("MOVING BETWEEN MATCHUPS — say you're moving on")
        out.append("This is heard, not read. A listener has no paragraph breaks to")
        out.append("see, so without a spoken handoff every matchup blurs into the")
        out.append("last one and they lose track of who you're even talking about.")
        out.append("Open each segment AFTER THE FIRST with a short transition, then")
        out.append("get straight into it. Vary them - the same phrase six times is")
        out.append("worse than none:")
        ...
        out.append("orients the listener, so do that immediately.")
